...
def get_copy(db: Session, copy_id: int):
    ...
    if result:
        logger.debug(
            f"📔 [CRUD] 获取书籍副本: ID: {copy_id} | 书籍 ID: {result.book_id} | 副本号: {result.barcode_number}"
        )
    return result
...
def get_copy_by_barcode(db: Session, isbn: str, barcode_number: int):
    ...
    result = (
        db.query(BookCopy)
        .join(Book)
        .filter(Book.isbn == isbn, BookCopy.barcode_number == barcode_number)
        .first()
    )
    return result
...
def get_book_available_copy(db: Session, book_id: int):
    ...
    result = (
        db.query(BookCopy)
        .filter(BookCopy.book_id == book_id, BookCopy.status == CopyStatus.AVAILABLE)
        .first()
    )
    return result
...
def create_book_copies(db: Session, book_id: int, total_copies: int):
    ...
    for i in range(1, total_copies + 1):
        copy = BookCopy(book_id=book_id, barcode_number=i, status=CopyStatus.AVAILABLE)
        db.add(copy)
    db.commit()
